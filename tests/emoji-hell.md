...
## 7. Long emoji lines — wrap stress

*These cells exceed 40 columns. This is a single-column table, so wrapping never applies (continuation lines would re-parse as new rows); each cell stays on one line and identical codepoints must survive realignment.*
