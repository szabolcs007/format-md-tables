...
import { MAX_WIDTH, check_alignment, check_content } from "./helpers.ts";
...
describe("bytes & ansi", () => {
  test("BOM preserved, CRLF preserved unmixed, alignment and content hold, ANSI survives, idempotent", () => {
    ...
    const { data, changed } = alignBytes(raw, MAX_WIDTH, 8);
