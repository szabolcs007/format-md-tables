...
## 7. Header/delimiter column-count mismatch is not a table

GFM requires the header row to have exactly as many cells as the delimiter row. Below the delimiter row defines one column while the header has two, so this is NOT a table — and neither is the pipe-less paragraph after it. All three lines must pass through byte-identical.

| head A | head B |
| ------ |
This line has no pipe and is not a table row; nothing here may move.
...
## 13. Blockquote prefixes on prose

Blockquote lines that are prose, not tables, stay untouched at any depth. The pipe below is paragraph content with no delimiter row after it.

> > > quoted prose with a | pipe inside
> > > second quoted line
